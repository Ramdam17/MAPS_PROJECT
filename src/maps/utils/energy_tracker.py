...
class NvidiaEnergyTracker:
    ...
    def __init__(
        ...
        if not self.has_nvidia_smi:
            raise RuntimeError(
                "nvidia-smi not found. This tracker requires NVIDIA GPUs with nvidia-smi."
            )
        ...
        self.emissions_factor = self.emissions_factors.get(
            country_iso_code, 0.47
    ...
    def _log_gpu_metrics(self):
        ...
        try:
            result = subprocess.run(
                [
                    "nvidia-smi",
                    "--query-gpu=index,power.draw,utilization.gpu,utilization.memory,memory.used,memory.total,temperature.gpu",
    ...
    def _calculate_total_energy(self):
        ...
        for i in range(1, len(self.time_readings)):
            time_diff = (
                self.time_readings[i] - self.time_readings[i - 1]
            ...
            power_start = sum(
                gpu.get("power_watts", 0) for gpu in self.gpu_readings[i - 1].values()
            )
    ...
    def _calculate_power_metrics(self):
        ...
        for idx, metrics in per_gpu_metrics.items():
            result["per_gpu_metrics"][idx] = {
                "avg_power_watts": np.mean(metrics["power_readings"])
                if metrics["power_readings"]
                else 0,
                "peak_power_watts": np.max(metrics["power_readings"])
                if metrics["power_readings"]
                else 0,
    ...
    def _generate_plots(self, df, timestamp):
        ...
        plot_path = os.path.join(
            self.output_dir, f"{self.project_name}_energy_plot_{timestamp}.png"
        )
        plt.tight_layout()
...
class MLModelEnergyEfficiency:
    ...
    def generate_efficiency_report(self, efficiency_metrics, output_dir=None):
        ...
        report_path = os.path.join(
            output_dir, f"{self.model_name}_efficiency_report_{timestamp}.md"
        )

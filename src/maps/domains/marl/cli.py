...
# Vendored from
#   external/paper_reference/marl_tmlr/onpolicy/scripts/train/train_meltingpot.py
# Keeps the MAPS factorial mapping (setting 1-6 -> meta on/off x cascade1/2).
# Import paths pointing at ported modules are fixed (env_creator -> maps...env,
# separated MeltingpotRunner -> maps...runner). The GENERIC MAPPO plumbing is NOT
# part of this per-domain rebuild and is left importing from onpolicy.*:
#   * onpolicy.config.get_config       (the ~400-line argparse)
#   * onpolicy.envs.env_wrappers       (SubprocVecEnv / DummyVecEnv)
#   * onpolicy.runner.shared...        (shared variant; we ported only `separated`)
# CODE-ONLY: needs meltingpot + that plumbing (absent here) — not runnable locally.
...
def main(args):
    ...
    if all_args.share_policy:
        from onpolicy.runner.shared.meltingpot_runner import MeltingpotRunner as Runner  # shared variant not ported
    else:
        from maps.domains.marl.runner import MeltingpotRunner as Runner

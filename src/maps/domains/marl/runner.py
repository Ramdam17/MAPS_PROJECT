...
#   external/paper_reference/marl_tmlr/onpolicy/runner/separated/meltingpot_runner.py
# Changes vs source (D16.4, user choice "copie quasi a l'identique"):
#   * import paths onpolicy.* -> maps.domains.marl.*
#   * energy_tracker: the reference has NO energy_tracker module (it lived in the
#     student's repo, not external/paper_reference). We cannot reproduce it
#     faithfully, so we substitute inert no-op stubs so the instrumentation
#     call-sites (initialize_tracker / tracker.log_point / stop_get_results_tracker)
#     stay verbatim. Energy logging is therefore a no-op.
# CODE-ONLY: the MeltingpotRunner uses the meltingpot env (absent here) — not
# testable locally. Lint/format disabled to preserve the verbatim source.
# --------------------------------------------------------------------------------
import os
import numpy as np
...
import torch
import cv2
from maps.domains.marl.util import update_linear_schedule
...
# energy_tracker is ABSENT from external/paper_reference — inert no-op stubs keep the
# runner's instrumentation call-sites verbatim (see header note).
class NvidiaEnergyTracker:
    def __init__(self, *args, **kwargs):
        pass

    def log_point(self, *args, **kwargs):
        pass

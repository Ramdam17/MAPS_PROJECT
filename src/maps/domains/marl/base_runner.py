...
# Vendored from
#   external/paper_reference/marl_tmlr/onpolicy/runner/separated/base_runner.py
# Changes vs source (D16.4, user choice "copie quasi a l'identique"):
#   * import paths onpolicy.* -> maps.domains.marl.*
#   * __init__ construction sites (Policy / TrainAlgo / SeparatedReplayBuffer) are
#     rewired to our explicit-keyword constructors (16.C/16.G) instead of the
#     source's argparse-`args`-first signature — the ONLY unavoidable exception to
#     "imports only", because our policy/trainer/buffer take explicit params. Values
#     are pulled verbatim from all_args; the Meltingpot path is wired, the unused
#     non-Meltingpot / happo / hatrpo branches are left as-is (dead for our cli).
# CODE-ONLY: builds the meltingpot env stack (absent here) — not testable locally.
# Lint/format disabled to preserve the verbatim source.
# --------------------------------------------------------------------------------
import time
import os
import numpy as np
...
from maps.domains.marl.util import update_linear_schedule, get_shape_from_obs_space
...
class Runner(object):
    def __init__(self, config):
        ...
        if self.all_args.algorithm_name == "happo":
            ...
            from maps.domains.marl.trainer import R_MAPPO as TrainAlgo
            from maps.domains.marl.rmappo_policy import R_MAPPOPolicy as Policy
        ...
        for agent_id in range(self.num_agents):
            if not self.env_name == "Meltingpot":
                ...
                po = Policy(
                    obs_space=self.envs.observation_space[player_key]['RGB'],
                    cent_obs_space=share_observation_space,
                    act_space=self.envs.action_space[player_key],
                    hidden_size=self.all_args.hidden_size,
                    recurrent_n=self.all_args.recurrent_N,
                    cascade_iterations_1=self.all_args.cascade_iterations1,
                    cascade_iterations_2=self.all_args.cascade_iterations2,
                    lr=self.all_args.lr,
                    critic_lr=self.all_args.critic_lr,
                    opti_eps=self.all_args.opti_eps,
                    weight_decay=self.all_args.weight_decay,
                    optimizer=self.all_args.optimizer,
                    use_orthogonal=self.all_args.use_orthogonal,
                    gain=self.all_args.gain,
                    device=self.device,
                )
                # policy network
        ...
        for agent_id in range(self.num_agents):
            ...
            tr = TrainAlgo(
                ...
                hidden_size=self.all_args.hidden_size,
                clip_param=self.all_args.clip_param,
                ppo_epoch=self.all_args.ppo_epoch,
                num_mini_batch=self.all_args.num_mini_batch,
                data_chunk_length=self.all_args.data_chunk_length,
                value_loss_coef=self.all_args.value_loss_coef,
                entropy_coef=self.all_args.entropy_coef,
                max_grad_norm=self.all_args.max_grad_norm,
                huber_delta=self.all_args.huber_delta,
                use_attention=self.all_args.use_attention,
                use_max_grad_norm=self.all_args.use_max_grad_norm,
                use_clipped_value_loss=self.all_args.use_clipped_value_loss,
                use_huber_loss=self.all_args.use_huber_loss,
                use_popart=self.all_args.use_popart,
                use_valuenorm=self.all_args.use_valuenorm,
                use_value_active_masks=self.all_args.use_value_active_masks,
                use_policy_active_masks=self.all_args.use_policy_active_masks,
                device=self.device,
            )
            # buffer
            if not self.env_name == "Meltingpot":
                ...
                bu = SeparatedReplayBuffer(
                    ...
                    num_actions=self.envs.action_space[player_key].n,
                    use_valuenorm=self.all_args.use_valuenorm,
                )

...
| D-002 | First-order loss | "Contrastive loss" (eq.4, Chen et al. 2020 SimCLR form) | Contractive AutoEncoder loss (Rifai et al. 2011) — both are sometimes called "contrastive" in older literature | `cae_loss` (parity with student, decision pending) | `maps.components.losses.cae_loss` (SimCLR variant not yet implemented). **C.7 deep dive (2026-04-19):** paper eq.4 is explicitly SimCLR/NT-Xent (Chen 2020); student docstring uses "contrastive" phraseology but implements CAE. Mathematically distinct losses. Decision on whether to add a paper-faithful SimCLR variant → **sub-phase D.17** of the Sprint-08 plan. See `docs/reviews/losses.md §C.7` for full analysis. **Note cascade interaction (C.5):** the cascade integrates 50 iterations over the 2nd-order comparator; Blindsight/AGL + SARL 2nd-order have dropout-in-cascade → cascade averages 50 dropout masks (Monte-Carlo dropout-like regularization). SARL 1st-order has no dropout → cascade is mathematically a no-op on deterministic forward (see `docs/reviews/cascade.md §(d)` + `docs/reviews/second_order.md §C.5 (d)`). |
...
| D-sarl-batch-size            | `sarl/training_loop.py:74` `BATCH_SIZE`     | 32 (text)                  | 32 (L99)                         | 128 **→ 32 aligned 2026-04-20** (D.9, port-only divergence fixed) | ✅ resolved (new) | D.9 done — override `-o training.batch_size=128` for prior port behaviour |
| D-sarl-step-size-1           | `sarl/training_loop.py:80` `STEP_SIZE_1`    | 0.00025                    | 0.00025                          | 0.0003 **→ 0.00025 aligned 2026-04-20** (D.9, port typo fixed) | ✅ resolved (new) | D.9 done — override `-o optimizer.lr_first_order=0.0003` for prior port behaviour |

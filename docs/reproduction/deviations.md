...
|------------------------------|---------------------------------------------|----------------------------|---------------------------------|----------------------------------------|:-------:|:---------------------|
| **D-sarl-wrong-variant**     | `src/maps/experiments/sarl/model.py` (whole module) + Sprint-04b parity reference | `maps_v1.py` is the canonical paper variant (invoked by `SARL_Training_Standard.sh base=2000000 -ema 25`); structural diffs are load-bearing: Q-head `Linear(1024, actions)` (NOT 128), dedicated decoder `Linear(128, 1024)` (NOT tied-weight), `comparison_layer` active in SecondOrderNetwork (NOT commented out), cascade target = 1024-dim Output (NOT 128-dim Hidden) | Port and parity reference target `maps_v2.py` after commit `8aa1138` (TD-008) renamed it to `maps.py` and deleted `maps_v1.py`. TD-008 audit was a token-presence comparison, NOT a structural diff — and was wrong. | **Wrong variant. ALL Phase F SARL/SARL+CL runs to date use v2 architecture at 500k frames; paper Table 6 numbers come from v1 at 2M frames.** | 🚨🚨 CRITICAL | **Sprint-09 — port `maps_v1.py` into `src/maps/experiments/sarl/model_v1.py`, retract TD-008, archive Phase F.4 v2 runs as "v2 reference", re-submit Phase F on Narval with v1 at 2M frames.** Full diff: `docs/reproduction/sarl-v1-vs-v2.md` (commit `6c59744`). |
...
|-------------------------------------|--------------------------------------|-----------------------------|--------------------------------|-------------------------------|:-------:|:--------------------|
| D-cl-weights                        | `config.cl.{weight_task,weight_distillation,weight_feature}` | T.11 (0.3, 0.6, 0.1) ≠ text p.17 "optimal" (0.4, 0.4, 0.2) | CLI `--weight1=40 --weight2=40 --weight3=20` → (0.4, 0.4, 0.2) | **(0.4, 0.4, 0.2)** paper text p.17 "optimal weights" (re-aligned 2026-05-19 Sprint-08 Phase F.4 ; Figure 7 reproduction target — text/Figure 7 wins over T.11 because Figure 7 IS the headline CL result) | ✅ resolved | 2026-05-19 reversal of D.20 decision — Phase F.4 curriculum sbatch (`sarl_cl_curriculum_narval.sh`) reproduces Figure 7, so we must use the weights that produced Figure 7. Override `-o cl.weight_task=0.3 -o cl.weight_distillation=0.6 -o cl.weight_feature=0.1` for T.11 variant, or `1.0/1.0/1.0` for student baseline |
...
|--------------------------------------|-----------------------------------------|-----------------------|--------------------------------------------|----------------------------|:-------:|:------------------------------------------|
| D-marl-hidden-size                   | `config.model.hidden_size`              | 100                   | 144 (config L221) / shell var              | **100** (paper, in marl.yaml)            | ✅ resolved (E.7 / Sprint-09 9.5)   | done — port aligned to paper. Override `-o model.hidden_size=144` for student baseline. |
| D-marl-actor-lr                      | `config.optimizer.actor_lr`             | 7e-5                  | config 7e-5 BUT shell `--lr 0.00002` = 2e-5 | **7e-5** (paper, in marl.yaml)              | ✅ resolved (E.7 / Sprint-09 9.5)   | done — port matches paper. Override `-o optimizer.actor_lr=2e-5` for student shell.                          |
| D-marl-critic-lr                     | `config.optimizer.critic_lr`            | **100** (typo)        | config 7e-5 (= actor_lr)                   | **7e-5** (student real, paper typo noted in marl.yaml) | ✅ resolved (E.7 / Sprint-09 9.5)   | done — port uses 7e-5 (paper "100" is a typo, see marl.yaml header).                |
| D-marl-num-env-steps                 | `config.training.num_env_steps`         | 15e6 (Table 12 typo) / 300k (§4 text) | config 40e6 / text p.15 says 300k          | **300k** (paper §4 text, in marl.yaml)      | ✅ resolved (E.17b3 / Sprint-09 9.5) | done — port matches paper §4 text. E.17b3 validated 3-seed at 300k reproducing paper Table 7 baseline `commons_harvest_closed` within 1.4σ.                     |
| D-marl-entropy-coef                  | `config.ppo.entropy_coef`               | 0.01                  | config 0.01 BUT shell `--entropy_coef 0.004` | **0.01** (paper, in marl.yaml)              | ✅ resolved (E.7 / Sprint-09 9.5)   | done — port matches paper. Override `-o ppo.entropy_coef=0.004` for student shell.                              |

...
| TD-008 | ~~SARL/MinAtar/examples/maps_v1.py vs maps_v2.py~~ **RETRACTED 2026-05-24** | Reproducibility | **CRITICAL** | High | ~~Two near-identical MAPS DQN variants~~ **WRONG: variants are structurally distinct; v1 is the paper-canonical entrypoint per `SARL_Training_Standard.sh base=2000000 -ema 25`. v2 differs on Q-head input dim (128 vs 1024), reconstruction decoder (tied weights vs dedicated `Linear(128, 1024)`), `comparison_layer` (commented out vs active), cascade target (Hidden 128 vs Output 1024). The whole SARL port + Sprint-04b parity reference targets v2 = WRONG VARIANT.** See `docs/reproduction/sarl-v1-vs-v2.md` (commit `6c59744`). Tracked as `D-sarl-wrong-variant` (CRITICAL) in deviations.md. Resolution: Sprint 09 — port v1, retract this audit, re-run Phase F on Narval at 2M frames.|

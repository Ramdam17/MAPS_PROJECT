...
def calculate_wagering_accuracy(output_second, targets_wagering):
    """
    Calculate wagering accuracy from model predictions and targets.
    
    Parameters:
    - output_second: Model predictions (logits, before sigmoid)
    - targets_wagering: Ground truth targets (0 or 1)
    
    Returns:
    - accuracy: The accuracy of wagering predictions
    """
    # Ensure inputs are torch tensors
    if not isinstance(output_second, torch.Tensor):
        output_second = torch.tensor(output_second, dtype=torch.float32)
    if not isinstance(targets_wagering, torch.Tensor):
        targets_wagering = torch.tensor(targets_wagering, dtype=torch.float32)
    
    # Convert logits to binary predictions (0 or 1)
    predictions = (torch.sigmoid(output_second) >= 0.5).float()
    
    # Calculate accuracy
    correct = (predictions == targets_wagering).float().sum()
    total = targets_wagering.numel()
    ...
    with torch.autograd.set_detect_anomaly(True):
        ...
        if meta:
            ...
            if previous_loss != None and (not(evaluation_continuous_learning)):
                ...
                accuracy_second = calculate_wagering_accuracy(output_second, targets_wagering)
            ...
            if previous_loss != None and (not(evaluation_continuous_learning)):
                return loss, loss_second, accuracy_second, (loss_task, loss_previous_task, feature_loss), (task_loss_second, previous_task_loss_second, feature_loss_second)
            else: 
...
def evaluation(env, meta, t, target_off, target_net, alpha, cascade_iterations_1, cascade_iterations_2, replay_start_size, num_actions, replay_off, r_buffer):
       ...
       if t % TRAINING_FREQ == 0 and sample is not None:
           ...
           if meta: 
               if previous_loss != None and (not(evaluation_continuous_learning)):
                   ...
                   loss_first, loss_second, accuracy_2net, (loss_task, previous_task_loss, feature_loss), (task_loss_second, previous_task_loss_second, feature_loss_second) = train(sample, policy_net, meta, alpha, cascade_iterations_1, cascade_iterations_2, False)
                   ...
                   loss_first, loss_second, accuracy_2net = train(sample, policy_net, meta, alpha, cascade_iterations_1, cascade_iterations_2, False)

               # Accumulate total losses
               loss_one += loss_first.item()
               loss_two += loss_second.item()
               accuracy_second += accuracy_2net
   ...
   if meta:
       if loss_two != None:
           ...
           accuracy_second = accuracy_second / counter_episode
   ...
   if previous_loss != None and (not(evaluation_continuous_learning)):
       ...
       return G, loss_one, loss_two, accuracy_second , (loss_one_task, loss_one_distillation, loss_one_feature), (loss_two_task, loss_two_distillation, loss_two_feature)
   else:
       # Return basic losses for standard learning
       return G, loss_one, loss_two, accuracy_second
...
def dqn(env, replay_off, target_off, output_file_name, meta, alpha, cascade_iterations_1, cascade_iterations_2, game_name, curriculum=False, curriculum_evaluation=False, store_intermediate_result=True, load_path=None, checkpoints=False, adapnet=False):
    ...
    if game_name=='freeway':
        ...
        checkpoint_iteration = TARGET_NETWORK_UPDATE_FREQ
    ...
    # Count parameters for policy net and second order net
    ...
    if meta:
        ...
        second_order_net_params = count_parameters(second_order_net)
        total_params = total_params + second_order_net_params
    else:
    ...
    if load_path is not None and isinstance(load_path, str):
        ...
        if meta:
            ...
            accuracy_twos_init = checkpoint['training_accuracy_two']
            validation_accuracy_two_init = checkpoint['validation_accuracy_two'] 
    ...
    accuracy_twos = accuracy_twos_init
    validation_accuracy_two = validation_accuracy_two_init
    ...
    while ((t < (NUM_FRAMES - reduce_curriculum)) and (not is_curriculum_terminated)) or (curriculum_evaluation and (evaluation_curriculum_counter < number_validation_curriculum)):
            ...
            if t % TRAINING_FREQ == 0 and sample is not None:
                ...
                if meta: 
                    if previous_loss != None and (not(evaluation_continuous_learning)):
                        ...
                        loss_first, loss_second, accuracy_2net, (loss_task, previous_task_loss, feature_loss), (task_loss_second, previous_task_loss_second, feature_loss_second) = train(sample, policy_net, meta, alpha, cascade_iterations_1, cascade_iterations_2, True)
                    
                    else:
                        # Standard training without distillation
                        loss_first, loss_second, accuracy_2net= train(sample, policy_net, meta, alpha, cascade_iterations_1, cascade_iterations_2, True)
        ...
        if meta:
            ...
            accuracy_twos.append(accuracy_second)
        ...
        if e % checkpoint_iteration == 0 or curriculum_evaluation:
            ...
            for _ in range(validation_iterations):
                ...
                if curriculum_evaluation:
                    ...
                    val_reward, val_loss_one, val_loss_two , val_accuracy_two= evaluation(
                    ...
                    val_reward, val_loss_one, val_loss_two, val_accuracy_two, (loss_one_task, loss_one_distillation, loss_one_feature), (loss_two_task, loss_two_distillation, loss_two_feature) = evaluation(
                        env, meta, random_number, target_off, target_net, alpha,
                        cascade_iterations_1, cascade_iterations_2, replay_start_size, num_actions, replay_off,
                        r_buffer)
                ...
                v_accuracies_two.append(val_accuracy_two)
            ...
            if meta:
                ...
                validation_acc_two = numpy.mean(v_accuracies_two)
                validation_acc_two_std = numpy.std(v_accuracies_two)
                validation_accuracy_two.append(validation_acc_two)
                 
            else: 
                validation_loss_two = None
                validation_loss_two_std = None

                validation_acc_two = None
                validation_acc_two_std = None
            ...
            if curriculum_evaluation:
                ...
                if loss_one!=None and loss_two!=None:
                    print("Episode " + str(e) + " | Return: " + str(G) + " | Avg return: " +
                            ...
                            str(numpy.around(loss_two,5)) + " ( val "+str(numpy.around(validation_loss_two,5)) +" )" + 
                            " | acc second order: " + str(numpy.around(accuracy_second,5)) + " ( val "+str(numpy.around(validation_acc_two,5)) +" )" + 
                            " | Frame: " + str(t)+" | Time per frame: " +str( numpy.around( (time.time()-t_start)/t  ,5)) )
                ...
                if checkpoints:
                    torch.save({
                                ...
                                'training_accuracy_two':accuracy_twos,
                                'validation_accuracy_two':validation_accuracy_two,
